...
class AppStore extends Component {
  ...
  getFilterItems = searchApp => {
    const {activeTabId} = this.state
    const filterDetail = searchApp.filter(each => each.category === activeTabId)
    return filterDetail
  }
  ...
  render() {
    ...
    const searchResults = this.getSearchResultsA()
    const filterItems = this.getFilterItems(searchResults)
        ...
        <div className="bg-container">
          ...
          <ul className="ul-container">
            {tabsList.map(eachTodo => (
              <TabItem
                key={eachTodo.tabId}
                TabDetailList={eachTodo}
                onclickTabItem={this.onclickTabItem}
                isActive={activeTabId === eachTodo.tabId}
              />
            ))}
          </ul>
          <ul className="app-detail-container">
            {filterItems.map(eachOf => (
              <AppItem key={eachOf.appId} detailOfAppItem={eachOf} />
            ))}
          </ul>
        </div>
      </>
